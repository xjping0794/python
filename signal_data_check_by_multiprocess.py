#!/usr/bin/env pypy
# -*-coding:utf-8 -*-
# Script Name           : signal_data_check_by_multiprocess.py
# Author                    : xjping0794
# Created                   : 9 May 2017
# Last Modified         :
# Version                   : 2.6
...
def main(loops,lineno,endline):
    logging.info('进程%d处理的文件开始行数: %d,结束行数: %d\n' % (loops + 1, lineno+1,endline))
...
if __name__ == "__main__":
    ...
    while statsmin <= endmin:
        ...
        try:
            ...
            fo.close ()
            process_num = 8
            per_process_lines=len(lines)/process_num
            for i in range(process_num):
                if process_num-i==1:
                    t=Process(target=main,args=(i,i*per_process_lines,len(lines)))
                else:
                    t=Process(target=main,args=(i,i*per_process_lines,(i+1)*per_process_lines))
                processes.append(t)
            for i in range(process_num):
                processes[i].start()
            for i in range(process_num):
