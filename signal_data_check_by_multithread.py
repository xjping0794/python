...
import re,logging,time,datetime,threading
from ftplib import FTP

CONFIG_FILE_NAME="/data2/http/bin/signal_field.cfg"
statsmin="201705172109"
endmin="202001012359"
...
def main(loops,lineno,endline):
    logging.info('线程%d处理的文件开始行数: %d,结束行数: %d\n' % (loops + 1, lineno+1,endline))
    for line in lines[lineno:endline]:
        lineno += 1
        # 校验字段数
        if not datacheck.check_field_num (cffieldnum, len (line.split ("|"))):
            ...
            continue
        # 校验电话号码
        datacheck.check_msisdn_valid (downloadfile, lineno,
                                      line.split ("|")[int (cf_msisdn_row_position)].rstrip ())
        # 校验时间
...
if __name__ == "__main__":
    ...
    while statsmin <= endmin:
        ...
        try:
            ...
            fo.close ()
            threads_num = 8
            per_thread_lines=len(lines)/threads_num
            print per_thread_lines
            for i in range(threads_num):
                if threads_num-i==1:
                    t=threading.Thread(target=main,args=(i,i*per_thread_lines,len(lines)))
                else:
                    t=threading.Thread(target=main,args=(i,i*per_thread_lines,(i+1)*per_thread_lines))
                threads.append(t)
            for i in range(threads_num):
                threads[i].start()
            for i in range(threads_num):
